...
Sensitive configuration files must be **gitignored** and must be created from templates before running. Make sure to add them to gitignore.
...
#### `map.tsx` — Map Screen
- Uses `react-native-webview` with **Leaflet.js** loading **OpenStreetMap** tiles (replacing `react-native-maps`).
- Header with back button ("< Back"), title "Video Locations".
- Displays only videos that have valid coordinates (`hasCoordinates` check).
- **WebView map**: An HTML page is rendered inside a `WebView` containing Leaflet.js (loaded from CDN). Markers are added to the Leaflet map via `webViewRef.injectJavaScript()`.
- **Auto-fit bounds**: After loading, the map automatically pans and zooms to fit all markers using Leaflet's `fitBounds()`.
- **Marker interaction**: Tapping a marker posts `{ type: 'markerClick', videoId }` via `ReactNativeWebView.postMessage`. React Native receives it via `onMessage` and displays a **bottom panel** (plain `Animated.View` positioned absolutely at the bottom) containing:
  - Video thumbnail
  - Title, channel name
...
- Bottom panel is dismissible by tapping a dimmed backdrop or a close button.
- **User-Agent**: Set to `dev.elainedb.{package_name}/1.0` per OSM tile usage policy.
...
- **User-Agent identification:** Set a unique, identifiable `User-Agent` header (e.g., `dev.elainedb.rn_claude/1.0`) on all tile requests. This is **mandatory** — requests without a proper User-Agent are rejected. In the Leaflet.js WebView, configure this via the `WebView`'s `applicationNameForUserAgent` prop or by setting request headers in the HTML page.
- **Tile caching:** Leaflet and the WebView's built-in HTTP cache handle tile caching automatically. Do not send `Cache-Control: no-cache` or `Pragma: no-cache` headers.
- **Rate limiting:** Do not aggressively prefetch tiles. Rely on Leaflet's default on-demand tile loading as the user pans/zooms. Avoid programmatic tile prefetch loops.
...
| `expo-sqlite` ~14 | SQLite local database (must use v14 for Expo SDK 54 compatibility — see 13.4) |
...
| `react-native-webview` | WebView for Leaflet.js map rendering |
...
### 13.4 expo-sqlite Version Mismatch with Expo SDK 54

**Problem:** Installing `expo-sqlite` without pinning the version pulls v55, which depends on a newer Expo Kotlin runtime that does not exist in Expo SDK 54. This causes a `NoSuchMethodError` at runtime when the native module tries to call methods from the newer runtime.

**Fix:** Pin `expo-sqlite` to v14, which is the version compatible with Expo SDK 54:
...
**Problem:** `react-native-maps` with OpenStreetMap `UrlTile` overlays is unreliable: custom tile headers (User-Agent, Referer) are inconsistently applied across platforms, the default Google Maps base layer shows through under OSM tiles, and `@gorhom/bottom-sheet` gesture handlers conflict with map touch events (see 13.3).

**Fix:** Replace `react-native-maps` with `react-native-webview` + Leaflet.js loading OpenStreetMap tiles directly:

- Use `react-native-webview` to render an HTML page containing Leaflet.js (loaded from CDN).
- Leaflet renders markers natively; marker clicks communicate back to React Native via `ReactNativeWebView.postMessage`.
- Replace `@gorhom/bottom-sheet` with a plain `Animated.View` for the video detail panel to avoid gesture handler conflicts.
- Set the User-Agent to `dev.elainedb.{package_name}/1.0` per the OSM tile usage policy.

This approach eliminates the dependency on `react-native-maps`, `@gorhom/bottom-sheet`, `react-native-gesture-handler`, and `react-native-reanimated` from the map screen. Update the dependencies table accordingly:

| Remove from map screen | Replace with |
|---|---|
| `react-native-maps` | `react-native-webview` |
| `@gorhom/bottom-sheet` | Plain `Animated.View` |
| `react-native-gesture-handler` (map only) | Not needed for map |
| `react-native-reanimated` (map only) | Not needed for map |
